...
typedef struct {
  ...
  int headatend_fired;
  int decoder_done;
  uint32_t starved_calls;   /* callbacks seguidos que não enfileiraram nada */
...
static int g_audio_trace = -1;

static int audio_trace_enabled(void) {
  if (g_audio_trace < 0)
    g_audio_trace = getenv("HC_AUDIO_TRACE") ? 1 : 0;
  return g_audio_trace;
}
...
/*
 * FMOD already feeds OpenSL with its final stereo master mix.  The inherited
 * multi-player shim attenuated every stereo source to 80% and then applied a
 * second 30% master gain, leaving Oceanhorn at only 24% of the PCM generated
 * by FMOD.  Keep a little configurable makeup gain here; the soft limiter
 * below protects loud SFX from hard clipping.
 */
static float ocean_audio_master_gain(void) {
  static int initialized;
  static float gain;
  if (!initialized) {
    const char *value = getenv("HC_AUDIO_GAIN");
    gain = value && *value ? strtof(value, NULL) : 1.5f;
    if (!(gain >= 0.1f && gain <= 4.0f)) gain = 1.5f;
    fprintf(stderr, "[AUDIO] ganho de saída = %.2fx\n", gain);
    initialized = 1;
  }
  return gain;
}
...
static void sdl_audio_callback(void *userdata, Uint8 *stream, int len) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    ...
    /* Mono is copied to both outputs, so use the conventional -3 dB pan
       compensation. Stereo is already the final FMOD mix: do not attenuate. */
    if (src_channels == 1) vol *= 0.70710678f;
    player_vol[i] = vol;
  ...
  const float master_gain = ocean_audio_master_gain();
  ...
  /* OCEAN_AUDIOLOG=N: a cada N callbacks, registra o nível real entregue ao SDL.
   * Serve para provar que há música/efeito saindo (peak > 0) sem depender de
   * ouvir o aparelho. Desligado por padrão: o default não faz trace por frame. */
  {
    static int audio_log_every = -1;
    if (audio_log_every < 0) {
      const char *v = getenv("OCEAN_AUDIOLOG");
      audio_log_every = v ? atoi(v) : 0;
      if (audio_log_every < 0) audio_log_every = 0;
    }
    if (audio_log_every && (callback_count % (unsigned)audio_log_every) == 0) {
      float out_peak = 0.0f;
      for (int s = 0; s < out_samples; s++) {
      ...
      fprintf(stderr,
              "[AUDIOLVL] cb#%u mixpeak=%.0f outpeak=%.0f/32767 players=%d\n",
              callback_count, mix_peak, out_peak, num_active);
      for (int a = 0; a < num_active; a++) {
        int pi = player_active_list[a];
        AudioPlayer *pp = &g_players[pi];
        fprintf(stderr,
                "[AUDIOLVL]   p%d vol=%.3f peak=%.0f ch=%u rate=%u readable=%u "
                "state=%d\n",
                pi, player_vol[pi], player_peak[pi], pp->num_channels,
                pp->sample_rate, ring_readable(pp), (int)pp->play_state);
      }
      fflush(stderr);
    }
  }
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (&g_players[i].bq_ptr == itf_ptr) {
      ...
      if (audio_trace_enabled() &&
          (p->enqueue_counter < 16 || (p->enqueue_counter % 512) == 0)) {
        const int16_t *pcm = (const int16_t *)pBuffer;
        uint32_t samples = size / sizeof(*pcm);
        int peak = 0;
        uint32_t nonzero = 0;
        for (uint32_t sample = 0; sample < samples; sample++) {
          int value = pcm[sample];
          int magnitude = value < 0 ? -value : value;
          if (magnitude > peak) peak = magnitude;
          if (value != 0) nonzero++;
        }
        fprintf(stderr,
                "[SLPCM] p%d enqueue=%u bytes=%u nonzero=%u/%u peak=%d\n",
                i, p->enqueue_counter + 1, (unsigned)size, nonzero,
                samples, peak);
      }
...
/* Quantos callbacks seguidos sem enqueue antes de aceitar que a fonte acabou.
 * O pump roda a ~4 ms, então 250 ≈ 1 s de silêncio real — folgado o bastante para
 * o mixer do FMOD e curto o bastante para o HEADATEND de um sample de verdade. */
#define OCEAN_STARVE_LIMIT 250u
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    ...
    while (p->callback && readable <= refill_threshold && max_calls > 0) {
      ...
      if (p->ever_enqueued && p->enqueue_counter == counter_before) {
        /* 🔊 O mixer do FMOD é uma fonte CONTÍNUA: ele pode não ter bloco pronto
         * numa chamada isolada (timing do mixer, não fim de áudio). Declarar
         * decoder_done no PRIMEIRO callback vazio silencia o jogo inteiro depois
         * dos buffers de priming — sintoma: um som curto ao abrir e mais nada.
         * Só considerar terminado após muitas tentativas seguidas sem dado. */
        if (++p->starved_calls >= OCEAN_STARVE_LIMIT) {
          if (!p->decoder_done) {
            p->decoder_done = 1;
            fprintf(stderr, "[SL] player %d decoder_done apos %u callbacks vazios\n",
                    i, p->starved_calls);
          }
        }
        break;   /* sai deste ciclo do pump; volta no próximo tick */
      }
      p->starved_calls = 0;   /* veio dado: a fonte está viva */
