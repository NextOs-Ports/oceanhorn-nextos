...
static int sem_trace(void) {
  static int initialized, enabled;
  if (!initialized) {
    enabled = getenv("HC_SEM_TRACE") != NULL;
    initialized = 1;
  }
  return enabled;
}
...
int sh_sem_init(void *s, int pshared, unsigned value) {
  (void)pshared;
  if (sem_trace() && g_n_init++ < 4)
    fprintf(stderr, "[SEM] init %p val=%u\n", s, value);
  pthread_mutex_lock(&g_sems_lock);
...
int sh_sem_wait(void *s) {
  ...
  if (!m) return -1;
  if (sem_trace() && g_n_wait++ < 60)
    fprintf(stderr, "[SEM] wait %p tid=%d count=%d\n",
            s, stid(), m->count);
...
int sh_sem_timedwait(void *s, const struct timespec *abs) {
  ...
  if (abs &&
    ...
    if (sem_trace() && normalized_count++ < 8)
      fprintf(stderr,
...
int sh_sem_post(void *s) {
  struct mysem *m = sem_lookup(s, 1, 0);
  if (!m) return -1;
  if (sem_trace() && g_n_post++ < 60)
    fprintf(stderr, "[SEM] post %p tid=%d\n", s, stid());
