 ...
 * O Rewired enumera o device por InputDevice.getMotionRanges(): lista VAZIA =
 * "controle sem eixo" e ele descarta o pad. Publicamos as 8 faixas de um pad
 * Android normal, com iterator e getMotionRange(axis) funcionando.
 * IDs de MotionEvent.AXIS_*: X=0 Y=1 Z=11 RZ=14 HAT_X=15 HAT_Y=16 LTRIGGER=17 RTRIGGER=18 */
static const int g_axis_ids[8] = {0, 1, 11, 14, 15, 16, 17, 18};
static int g_motion_range[8];         /* uma sentinela por eixo */
static int g_motion_range_list;       /* a List<MotionRange> */
static int g_motion_range_iter;       /* Iterator da lista */
static int g_motion_iter_pos;
/* índice do eixo a partir da sentinela; -1 se o objeto não é um MotionRange */
static int motion_range_index(void *obj) {
  for (int i = 0; i < 8; i++) if (obj == (void *)&g_motion_range[i]) return i;
  return -1;
}

/* ---- MotionEvent injetado (eixos analógicos + d-pad) ---- */
struct ocean_motion_s { int action, source, deviceId, metaState, buttonState, flags;
...
/* faixa de um eixo do pad: gatilhos são 0..1, o resto -1..1 */
static float ocean_axis_min(int axis) { return (axis == 17 || axis == 18) ? 0.0f : -1.0f; }
static float ocean_axis_flat(int axis) { return (axis == 15 || axis == 16) ? 0.0f : 0.05f; }
  ...
  if (nm) {
    ...
    { int mr = motion_range_index(obj);
      if (mr >= 0) {
        ...
        if (strcmp(nm, "getMin") == 0)   return ocean_axis_min(axis);
        if (strcmp(nm, "getMax") == 0)   return 1.0f;
        if (strcmp(nm, "getRange") == 0) return 1.0f - ocean_axis_min(axis);
        if (strcmp(nm, "getFlat") == 0)  return ocean_axis_flat(axis);
...
static void *jni_CallObjectMethodV(void *env, void *obj, void *methodID,
  ...
  if (nm) {
    ...
    /* 🎮 InputEvent.getDevice() de INSTÂNCIA. Sem isto o evento não sabe a que
    ...
    if (obj == (void *)&g_motion_range_list) {
      if (strcmp(nm, "iterator") == 0) { g_motion_iter_pos = 0; return &g_motion_range_iter; }
      if (strcmp(nm, "get") == 0) {
        int i = va_arg(ap, int);
        return (i >= 0 && i < 8) ? (void *)&g_motion_range[i] : NULL;
      }
    }
    if (obj == (void *)&g_motion_range_iter && strcmp(nm, "next") == 0) {
      if (g_motion_iter_pos < 0 || g_motion_iter_pos >= 8) return NULL;
      return &g_motion_range[g_motion_iter_pos++];
    }
    if (obj == (void *)&g_gamepad_device) {
      ...
      /* Lista REAL de eixos: lista vazia faz o Rewired descartar o pad. */
      if (strcmp(nm, "getMotionRanges") == 0)  return &g_motion_range_list;
      if (strcmp(nm, "getMotionRange") == 0) {
        int axis = va_arg(ap, int);
        for (int i = 0; i < 8; i++)
          if (g_axis_ids[i] == axis) return &g_motion_range[i];
        return NULL;
      }
    ...
    /* ⚠️ Oceanhorn NÃO tem asset pack: o `data.unity3d` mora direto em
       assets/bin/Data do APK base. Devolver um caminho de pack faz o
       MountDataArchive receber um diretório e falhar ("was not parsed"),
       e o data archive nunca abre. OCEAN_PACKPATH escolhe a resposta:
         none    (default) -> NULL: não há pack, a engine usa o caminho normal
         archive           -> caminho REAL do bin/Data/data.unity3d
         assets            -> <gamedir>/assets (comportamento do Horizon Chase)
         dir               -> <gamedir> */
    if (strcmp(nm, "getAssetPackPath") == 0) {
      const char *mode = getenv("OCEAN_PACKPATH");
      if (!mode || !strcmp(mode, "none")) {
        debugPrintf("jni_shim: getAssetPackPath -> NULL (sem asset pack)\n");
        return NULL;
      }
      if (!strcmp(mode, "archive")) {
        void *s = make_game_path_jstring("bin/Data/data.unity3d");
        debugPrintf("jni_shim: getAssetPackPath -> \"%s\"\n", resolve_jstring(s));
        return s;
      }
      if (!strcmp(mode, "dir")) return make_game_path_jstring("");
...
static unsigned char jni_CallBooleanMethodV(void *env, void *obj,
  ...
  if (nm) {
    ...
    /* 🔊 PackageManager.hasSystemFeature(...) -> true.
       O FMOD Android escolhe a saída entre OpenSL e AudioTrack-Java consultando
       FEATURE_AUDIO_LOW_LATENCY (a string está na libunity). Respondendo false ele
       vai para o AudioTrack, que precisa de JVM/ART e não existe no so-loader:
       o init morre em "null output" e NENHUMA saída é aberta — o log fica sem
       nenhum slCreateEngine e o jogo toca no vácuo. Com true ele entra no
       OpenSL, que o nosso opensles_shim serve por cima do SDL. */
    if (strcmp(nm, "hasSystemFeature") == 0) {
      const char *feat = resolve_jstring(va_arg(ap, void *));
      debugPrintf("jni_shim: hasSystemFeature(\"%s\") -> 1\n", feat ? feat : "?");
      return 1;
    }
...
static jint jni_CallIntMethodV(void *env, void *obj, void *methodID,
  ...
  if (nm) {
    ...
    /* MotionRange: eixo, fonte e (via float) faixa. */
    { int mr = motion_range_index(obj);
      if (mr >= 0) {
        if (strcmp(nm, "getAxis") == 0)   return g_axis_ids[mr];
        if (strcmp(nm, "getSource") == 0) return 0x01000010;  /* SOURCE_JOYSTICK */
        return 0;
      } }
...
static void *jni_GetStaticObjectField(void *env, void *clazz, void *fieldID) {
  ...
  /* PackageManager.FEATURE_* são constantes String com o nome REAL da feature;
     o FMOD passa esse valor para hasSystemFeature(). Devolver o nome do campo
     funcionaria (respondemos true para tudo), mas o valor certo mantém o log
     legível e não engana quem for depurar. */
  if (nm && !strcmp(nm, "FEATURE_AUDIO_LOW_LATENCY"))
    return make_jstring("android.hardware.audio.low_latency");
  if (nm && !strcmp(nm, "FEATURE_AUDIO_PRO"))
    return make_jstring("android.hardware.audio.pro");
  if (nm && !strncmp(nm, "FEATURE_", 8))
    return make_jstring(nm);
...
static void run_runnable(void *env, void *runnable) {
  ...
  if (!h) { debugPrintf("jni_shim: runOnUiThread sem handle (r=%p)\n", runnable); return; }
  /* Existem DUAS pontes de proxy no Unity e elas não são intercambiáveis:
   *   - bitter.jnibridge.JNIBridge.invoke(long ptr, Class, Method, Object[])
   *     -> `ptr` é PONTEIRO NATIVO C++; o nativo faz `ldr x8,[ptr]` e chama
   *        ptr->vtable[2]. Vale para proxies criados por newInterfaceProxy.
   *   - ReflectionHelper.nativeProxyInvoke(long handle, String nome, Object[])
   *     -> `handle` é um GCHandle do C# (inteiro pequeno: 3, 11, 19, 43...).
   *        É a ponte do AndroidJavaProxy criado por newProxyInstance.
   * Entregar um GCHandle ao invoke do JNIBridge faz ele desreferenciar 43 como
   * ponteiro = SIGSEGV imediato. Escolher a ponte pela origem do proxy. */
  if (proxy_has_interface(runnable, PROXY_BRIDGE_REFLECTION)) {
    void *invoke = jni_find_native("nativeProxyInvoke");
    void *reflection_class = class_for("com/unity3d/player/ReflectionHelper");
    if (!invoke) { debugPrintf("jni_shim: runOnUiThread sem nativeProxyInvoke (handle=%ld)\n", h); return; }
    g_in_run++;
    debugPrintf("jni_shim: >> RODANDO Runnable via ReflectionHelper (handle=%ld) ...\n", h);
    ((void *(*)(void *, void *, long, void *, void *))invoke)(
        env, reflection_class, h, make_jstring("run"), &g_empty_args_sentinel);
  ...
  if (!invoke) { debugPrintf("jni_shim: runOnUiThread sem invoke (r=%p h=%ld natives=%d)\n", runnable, h, g_natives_count); return; }
  g_in_run++;
