/*
 * main.c — Oceanhorn: Chronos Dungeon 4.0b54
 * (Unity 2022.3.61f1 IL2CPP) → NextOS (arm64).
 *
...
static int my_alog_vprint(int prio, const char *tag, const char *fmt, va_list ap) {
  ...
  /*
   * A Unity repete este fallback de formato a cada RenderTexture temporária,
   * chegando a dezenas de escritas síncronas por frame. É uma capacidade
   * conhecida do Mali, não um erro novo; registre só as primeiras ocorrências.
   */
  if (strstr(message, "'R8_UNorm' is not supported")) {
    static unsigned r8_warnings;
    unsigned occurrence = __sync_add_and_fetch(&r8_warnings, 1);
    if (occurrence > 3) return 0;
  }

  fprintf(stderr, "[ALOG:%d %s] %s", prio, tag ? tag : "?", message);
...
static int hc_fbo_trace(void) {
  static int initialized, enabled;
  if (!initialized) {
    enabled = getenv("HC_FBO_TRACE") != NULL;
    initialized = 1;
  }
  return enabled;
}
...
static long (*sound_openstate_orig)(void *, int *, unsigned *, int *, int *);
static long sound_openstate_hook(void *sound, int *state, unsigned *percent,
                                 int *starving, int *disk_busy) {
  long result =
      sound_openstate_orig(sound, state, percent, starving, disk_busy);
  static unsigned calls;
  if (result != 0 || calls++ < 24) {
    fprintf(stderr,
            "[OPENSTATE] sound=%p result=%ld state=%d percent=%u "
            "starving=%d disk=%d\n",
            sound, result, state ? *state : -1, percent ? *percent : 0,
            starving ? *starving : -1, disk_busy ? *disk_busy : -1);
  }
  return result;
}
...
static long cs_hook(void *sys, void *data, int mode, void *exinfo, void *out) {
  const int original_mode = mode;
  int effective_mode = mode;
  int forced_resident = 0;

  /*
   * Unity 2022 opens clips with LoadType=Streaming as
   * CREATESTREAM|NONBLOCKING.  Unlike the older Unity/FM0D build used by
   * Terraria, createSound itself returns OK here and the later
   * FMOD-nonblocking worker changes the sound from LOADING to ERROR
   * (ERR_INTERNAL).  Retrying only after createSound fails can therefore
   * never run.
   *
   * Make the same proven stream->sample fallback before the first FMOD call
   * and also clear NONBLOCKING.  The Unity preload job is already off the
   * render thread; a synchronous resident load here avoids both broken FMOD
  ...
  int openmem =
      (effective_mode & 0x800) || (effective_mode & 0x10000000);
  int trace = getenv("HC_SOUND_TRACE") != NULL;
  if (forced_resident) {
    static int nf;
    int occurrence = nf++;
    if (occurrence < 4 || r != 0)
      fprintf(stderr,
              "[CSSPY] STREAM->RESIDENT mode=0x%x -> 0x%x result=%ld\n",
              (unsigned)original_mode, (unsigned)effective_mode, r);
    if (r != 0 || getenv("HC_SOUND_TRACE")) dbg_sync();
  }
  if (r != 0) {  /* FALHA: dump detalhado do data source */
    if (nfail++ < 40) {
      ...
      fprintf(stderr, "[CSSPY] FAIL createSound mode=0x%x stream=%d openmem=%d data=%p name=%s d[0]=0x%lx exinfo=%p -> %ld\n",
              (unsigned)effective_mode, (original_mode >> 7) & 1,
              openmem, data, nm, d0, exinfo, r);
  ...
  } else if ((trace && nok++ < 40) || (!trace && nok++ < 4)) {
    char nm[96];
    nm[0] = 0;
    if (data && !openmem && addr_readable((uintptr_t)data)) {
      ...
      if (valid && terminated) snprintf(nm, sizeof nm, " name=\"%.86s\"", s);
    }
    void *sound = NULL;
    if (out && addr_readable((uintptr_t)out)) sound = *(void **)out;
    fprintf(stderr,
            "[CSSPY] ok createSound mode=0x%x%s stream=%d data=%p%s "
            "exinfo=%p out=%p sound=%p\n",
            (unsigned)effective_mode,
            effective_mode != original_mode ? " (resident)" : "",
            (original_mode >> 7) & 1, data, nm, exinfo, out, sound);
  }
  return r;
}
...
int main(int argc, char **argv) {
  ...
  fprintf(stderr,
          "=== Oceanhorn: Chronos Dungeon 4.0b54 / "
  ...
  if (getenv("HC_SOUND_TRACE") || getenv("HC_STREAM_FALLBACK") ||
    ...
    /* RVA REDERIVADO para a libunity do Oceanhorn (2022.3.61f1): o 0x16eea48 era
  ...
  jni_shim_set_package("com.FDGEntertainment.OceanhornChronosDungeon.gp", 0);
  ...
  patch_got("__FD_SET_chk", (void *)my_FD_SET_chk);
    ...
    g_m_il2cpp = so_save();
    /* Oceanhorn usa REWIRED (não o GamepadInputSource do Horizon): os 17 RVAs de
     * hc_input_install apontam pro lugar errado nesta libil2cpp. Rewired lê
     * /dev/input/event* direto, então o pad físico real enumera sozinho no device.
     * Mantido desligado por OCEAN_HCINPUT até rederivar (se necessário) pra Oceanhorn. */
    if (getenv("OCEAN_HCINPUT") && !hc_input_install(g_il2cpp_base))
      fprintf(stderr, "[HCINPUT] falha ao instalar o perfil de input\n");
   ...
   * EGL fallback. This is the state read by libunity 2022.3.61f1 at
  ...
  int frame_limit =
      getenv("OCEAN_FRAME_LIMIT") ? atoi(getenv("OCEAN_FRAME_LIMIT"))
  ...
  if (inject && !getenv("OCEAN_NOPAD")) {
    ocean_input_init();
    fprintf(stderr, "[OCEANPAD] nativeInjectEvent=%p\n", inject);
  }
  ...
  for (int f = 0;
    ...
    /* pad físico -> KeyEvent/MotionEvent Android (o caminho do Rewired) */
    ocean_input_poll(env, &thiz, inject);
    if (g_exit_signal || hc_input_exit_requested() ||
        ocean_input_exit_requested()) break;
    ...
    if (tapkey && inject && f > 120) {
      ...
      if (phase == 0 || phase == 3) {
        ...
        /* deviceId TEM de casar com o device enumerado por getDeviceIds() (=1 com
           TER_GAMEPAD). Evento com deviceId=0 é órfão: o Rewired mapeia por device
           e descarta. OCEAN_PADID permite testar outro id. */
        g_hk_inject.deviceId = getenv("OCEAN_PADID") ? atoi(getenv("OCEAN_PADID")) : 1;
        g_hk_inject.repeat = 0; g_hk_inject.flags = 0;
        g_hk_inject.metaState = 0; g_hk_inject.scancode = 0; g_hk_inject.unicode = 0;
        /* assinatura REAL: nativeInjectEvent(InputEvent, int) — chamar sem o
           inteiro deixa o 4º registrador com lixo. */
        int ir = ((int (*)(void *, void *, void *, int))inject)(env, &thiz, hk_keyevent_object(), 0);
